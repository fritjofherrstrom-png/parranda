...
test("assessCityCandidateReadiness reports Barcelona candidate coverage without counting structural anchors as real places", () => {
  ...
  assert.equal(readiness.total_candidates, 109);
  assert.equal(readiness.real_place_count, 103);
  assert.equal(readiness.structural_count, 6);
  assert.equal(readiness.coordinate_ready_real_place_count, 103);
...
test("assessCityCandidateReadiness can evaluate only user-facing real-place candidates", () => {
  ...
  assert.equal(readiness.total_candidates, 103);
  assert.equal(readiness.real_place_count, 103);
