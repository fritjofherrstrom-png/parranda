...
test("service worker registration is root-scoped for nested city routes", () => {
  ...
  assert.match(sw, /const CACHE_NAME = "parranda-v28"/);
  assert.match(sw, /"\.\/script\.js\?v=31"/);
...
test("landing and city shells use root-absolute asset urls for deep routes", async () => {
  ...
  assert.doesNotMatch(cityShell, /href="styles\.css\?v=20"/);
  assert.doesNotMatch(cityShell, /src="script\.js\?v=31"/);
  ...
  try {
    ...
    assert.equal(response.status, 200);
    assert.match(response.body, /<link rel="stylesheet" href="\/styles\.css\?v=20" \/>/);
    assert.match(response.body, /<script src="\/script\.js\?v=31"><\/script>/);
