...
test("service worker registration is root-scoped for nested city routes", () => {
  ...
  assert.match(sw, /const CACHE_NAME = "parranda-v29"/);
  assert.match(sw, /"\.\/script\.js\?v=32"/);
...
test("landing and city shells use root-absolute asset urls for deep routes", async () => {
  ...
  assert.doesNotMatch(cityShell, /href="styles\.css\?v=21"/);
  assert.doesNotMatch(cityShell, /src="script\.js\?v=32"/);
  ...
  try {
    ...
    assert.equal(response.status, 200);
    assert.match(response.body, /<link rel="stylesheet" href="\/styles\.css\?v=21" \/>/);
    assert.match(response.body, /<script src="\/script\.js\?v=32"><\/script>/);
