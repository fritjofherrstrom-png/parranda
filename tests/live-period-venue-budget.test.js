...
test("without a selected date, a clocked source period stays in the Live week after recurrence integration", async () => {
  const { resolver, venues } = recordingResolver();
  const out = await collectAnchorEvents({
    anchor: ANCHOR,
    now: NOW,
    registry: [SOURCE],
    fetcher: fetcherFor([mapless("period-1", "Utställningsperiod", TODAY, "Galleriet", {
      start: "18:00", end: "20:00", endDate: "2026-09-27",
    })]),
    venueResolver: resolver,
  });
  assert.deepEqual(venues, ["Galleriet"]);
  assert.deepEqual(out.this_week.map((event) => event.id), ["period-1"]);
});
