...
| 6 | **Cache genuinely-sparse (`loaded:0`) results** — a sparse city re-hits Overpass every visit (only non-empty results are stored) | caching | efficiency | med / low |
| 7 | **Engine-compose path stamps `walking_validation: valid:true`** without running a budget gate (agnostic-route-output.js ~914) — an engine-composed day can present a multi-km dead leg as validated. Gated off today, but latent dishonesty | route | honesty | high / med |
...
**Route / dayflow engine — adequate.** Legacy in-module synthesizer is the DEFAULT and the only branch with a real walking-budget gate + proximity chaining; the `synthesizeVia:"engine"` path hardcodes `walking_validation valid:true` (honesty gap, gated off). Two parallel daypart-ordering implementations with different proximity logic; fixed 1.22 detour + 12 min/km can reach the user surface.
...
- **Caching & performance** — `place_structure` re-calls `openDataLoader(anchor)` (a cache hit, but a second call path); sparse `loaded:0` not cached (#6); no persistent disk (#319); O(n²) clustering on the hot path.
