...
- **Engine convergence wiring (gated):** `composeAgnosticRouteOutput` gained a `synthesizeVia: "engine"` path that maps the admitted candidates (`mapAdmittedSelectionToSourceCandidates` — joins the lossy combination `selected[]` back to the rich planner-role candidates to recover type/provenance/attribution, reconstructs honest low trust) into the engine's `sourceCandidates` and synthesizes via `generateAgnosticRecommendations`; legacy in-module synthesizer stays the default (existing tests/behaviour unchanged) and is staged for removal. The promotion gate (`evaluateAgnosticPromotion`) returns the synthesized route as the actual day route ONLY when calibration is `thin_usable`/`usable` + low/medium + every `capped_by_*` ∈ {external_only, derived_tz, partial_context, heuristic_walking} + the anchor resolved strongly (the intake already fails closed on weak resolves); otherwise baseline + diagnostic. Wired in `/api/route-recommendations` behind `PARRANDA_AGNOSTIC_ENGINE_COMPOSE` (or `agnostic_engine_compose=1`), default OFF — respects "no public flip without persistent geocode/Overpass cache". Also fixed: `isExternalStop` now recognizes `provisional:true` engine stops, so an all-source-backed route trips `capped_by_external_only_sources` → stays honestly `thin_usable`, never `usable`. **Still pending before a public flip:** persistent geocode/Overpass caching, daypart promoted into compose ordering, and retiring the legacy synthesizer.

- **Live supply turned ON in the deploy + persistent-capable cache:** the live OSM/Overpass loader (`open-data-loader.js`) and the Nominatim place resolver existed and worked, but `render.yaml` set **no env vars**, so in production both were `null` — the deployed app ran on curated catalog + city-owned source packs only, and every loader/source-backed PR was dark in prod. This PR adds a persistent-capable source cache (`server/place-candidates/source-cache.js`: TTL + in-flight de-dupe + optional file backing via `PARRANDA_CACHE_DIR`), wraps the Overpass loader in it (repeat/concurrent lookups for the same ~110 m anchor never re-hit Overpass; only non-error results are stored), and enables `PARRANDA_OPEN_DATA_LOADER` + `PARRANDA_PLACE_RESOLVER` + `PARRANDA_CACHE_DIR` in `render.yaml`. This satisfies the cache half of "no public flip without persistent caching" for low-volume field use; **cross-redeploy** persistence still needs a mounted Render disk pointed at `PARRANDA_CACHE_DIR`. Engine-compose promotion (`PARRANDA_AGNOSTIC_ENGINE_COMPOSE`) stays OFF until that disk lands. Still pending for production-grade multi-source supply: a **second open source family** (Wikidata/Wikivoyage) so cross-family consensus can lift candidates past the single-family `map` ceiling the evidence reducer enforces.

- **Second open source family (Wikidata) + Unicode entity-resolution fix:** adds `server/place-candidates/wikidata-source.js` — a typed `wikibase:around` source (curated P31/P279* place classes: museum/gallery/park/garden/castle/market; the noise classes archaeological-site/settlement/tourist-attraction are excluded after live probing) emitting an independent `open_knowledge` family (CC0), local-language labels first so names match local OSM. Composed with the Overpass loader and **background-warmed** (WDQS cold queries run ~10-30 s, so they NEVER block the route: on a cache miss the request gets OSM only and Wikidata warms out-of-band, appearing on the repeat visit). The keystone fix: `entity-resolution.js` `normalizeName` was ASCII-only (`[^a-z0-9]`), so a Greek/Cyrillic name normalized to "" → zero tokens → **non-Latin places could never merge on geo+name and cross-source consensus was impossible for non-Latin cities**. Now Unicode-aware (`\p{L}\p{N}`), ASCII behavior unchanged. With both, a place OSM and Wikidata each know (e.g. `Επιγραφικό μουσείο`) merges into one candidate carrying `map` + `open_knowledge` → real consensus past the single-family ceiling. Gated by `PARRANDA_WIKIDATA_SOURCE` (+ `PARRANDA_WIKIDATA_LABEL_LANGS`). **Honest scope:** Wikidata is a NOTABLE-culture source (museums/galleries/markets), not the everyday walkable fabric OSM covers; its coords are coarser and its coverage thinner, so it complements OSM rather than replacing it.
